		...
		switch( OptionsButtonGroup[?"cursor"] ){
			case 0: //music
				...
				if(mouse_check_button_pressed(mb_left)){
					if((mouse_x >= MenuFont_X+vol_line_X && mouse_x <= MenuFont_X+vol_line_X+vol_line_length)&&
				...
				if (MusicPressed){
					if((mouse_x >= MenuFont_X+vol_line_X && mouse_x <= MenuFont_X+vol_line_X+vol_line_length)&&
						(mouse_y>=MenuFont_Y-sprite_get_height(sVolPicker) && mouse_y<=MenuFont_Y+sprite_get_height(sVolPicker))){
						set_music_volume( (mouse_x - MenuFont_X - vol_line_X)/vol_line_length)
					}
				}
			...
			case 2: //Language
				...
				for(var i = 0; i < array_length(Language_option) ; i++){
					if(mouse_check_button_pressed(mb_left)){
						if(( mouse_x >= MenuFont_X + i*100 && mouse_x < MenuFont_X + (i+1)*100 )&&
